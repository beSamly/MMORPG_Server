...
void TestApp::Start() {
  ...
  TimeUtil::LogElapsedTimeInMilliSec(
      "float test = FLOAT_MIN 100000 time", [&]() -> void {
        for (int i = 0; i < 300000; i++) {
  ...
  /*
          ...
          DoResolveCollision 함수 1번 실행 시 0.0000016 sec  = 0.0016 ms
     소요해야하는데 현재는 0.013 ms 소요 -> 10배만 빨리지면 될 듯..
  */

  TimeUtil::LogElapsedTime("Calling DoResolveCollision() 1000 time",
                           [&]() -> void {
                             for (int j = 0; j < 60; j++) {
