...
class Vector3 {
 ...
 public:
  ...
  static Vector3 Project(Vector3 vector, Vector3 onNormal) {
    ...
    return Vector3(onNormal.x * num2 / num, onNormal.y * num2 / num,
                   onNormal.z * num2 / num);
  }
};
...
class CollisionTestUtil {
 public:
  ...
  static CollisionInfo CheckCollision_BetweenTriangleSphere(Triangle triangle,
                                                            Vector3 target,
                                                            float radius) {
    ...
    // 상태이다.
    if (FindSeperatingAxe_BetweenTriangleSphere(triangle.vertices, target,
                                                radius)) {
    ...
    // 처리.
    if (IsTargetLocatedOppositeSideFromTriangleNormal(
            triangle.GetCrossProduct(), first, target)) {
      return CollisionInfo();
    }

    if (IsPointInTriangle(first, second, third, target)) {
      float overlap = CalculateOverlap_BetweenPointSphere(
          triangle.GetCrossProduct(), first, target, radius);
      if (overlap > 0.0f) {
      ...
      {
        float overlap =
            IsSphereHangingOnLineOfTriangle(triangle, target, radius);
        if (overlap > 0.0f) {
          CollisionInfo collisionInfo;
          collisionInfo.penetrationDepth = overlap;
          collisionInfo.penetrationNormal = triangle.GetCrossProduct();
  ...
  static bool IsCollidedFromSlope(CollisionInfo collisionInfo) {
    if (abs(collisionInfo.penetrationNormal.y) > 0.01f &&
        (abs(collisionInfo.penetrationNormal.x) > 0.001f ||
         abs(collisionInfo.penetrationNormal.z) > 0.001f)) {
      return true;
    } else {
      return false;
    }
  ...
  static CollisionInfo ResolveSlope(CollisionInfo collisionInfo) {
    ...
    if (IsCollidedFromSlope(collisionInfo)) {
      ...
      float angleInDegree =
          std::acos(Vector3::DotProduct(upVector, penetraionNormal) /
                    (upVector.Magnitude()) * penetraionNormal.Magnitude());
      ...
      float newDepth =
          (collisionInfo.penetrationDepth /
           std::asin(anotherAngleInDegree));  // Sin(90도)는 무조건 1이니 생략
  ...
  // 역방향(backward)
  static float CalculateOverlap_BetweenPointSphere(Vector3 projectTo,
                                                   Vector3 targetA,
                                                   Vector3 targetB,
                                                   float radius) {
    // 설명
 ...
 private:
  static bool IsTargetLocatedOppositeSideFromTriangleNormal(Vector3 normal,
                                                            Vector3 targetA,
                                                            Vector3 targetB) {
  ...
  static float IsSphereHangingOnLineOfTriangle(Triangle t, Vector3 target,
                                               float radius) {
  ...
  static bool FindSeperatingAxe_BetweenTriangleSphere(vector<Vector3>& vertices,
                                                      Vector3 target,
                                                      float radius) {
    for (int i = 0; i < 3; i++) {
      ...
      if (min > (targetDotProduct + radius) ||
          max < (targetDotProduct - radius)) {
        return true;
      }
    }
...
class NavigationMeshAgent {
 ...
 public:
  ...
  void AddTriangle(string meshName, Triangle triangle) {
    mapMesh[meshName].AddTriangle(triangle);
  }

  void AddGridInfo(GridInfo gridInfo) {
    mapGridInfo[gridInfo.gridIndex] = gridInfo;
  }
  ...
  Vector3 ResolveCollision(Vector3 position, float radius,
    ...
    string gridIndex =
        row + "_" + column;  // std::format("{}_{}", row, column); c++ 20 latest
                             // 빌드에서만 작동
    ...
    for (AdjacentMeshInfo& adjacentMeshInfo : gridInfo.vecAdjacentMeshInfo) {
      ...
      if (isTerrain) {
        ...
        CollisionInfo terrainCollisionInfo = GetBestCollisionInfo_From_Terrain(
 ...
 private:
  ...
  // 리턴한다.
  vector<CollisionInfo> GetBestCollisionInfo_From_Object(
      Mesh mesh, vector<int>& triangleIndicesToCheck, Vector3 position,
      float radius) {
    ...
    for (auto& [_, vecCollisionInfo] : map_direction_to_vecCollision) {
      ...
      if (CollisionTestUtil::IsCollidedFromSlope(finalCollisionInfo)) {
        finalCollisionInfo =
            CollisionTestUtil::ResolveSlope(finalCollisionInfo);
      }
