...
namespace PhysicsEngine {
	...
	public:
		static DotProductType GetDotProductTypeBetween(Vector3 a, Vector3 b)
		{
			...
			// if (value == 0.0f) 로 비교하게 되면 0.00001f 같은 경우 거의 perendicular이지만 조건부에 타지 않으니 
			// 0.0f 보다는 크도 0.001f 보다 작은 경우는 perendicular 로 간주한다
		...
		static CollisionInfo CheckCollision_BetweenTriangleSphere(Triangle triangle, Vector3 target, float radius)
		{
			...
			/*----------------------------------------------------------------------------------------
				아래 함수 호출을 주석친 이유는 굳이 Seperating Axe 검사를 하지 않아도
				CalculateOverlap_BetweenPointSphere, IsSphereHangingOnLineOfTriangle 함수에서 걸러진다.
				혹시 나중에 제대로 작동 안 하면 주석 풀기
				History : 가끔 Unity prefab에 이상한 vertex a,b,c 가 한 라인에 있는 mesh가 딸려올 때가 있다.
				이경우 seperating axe 검사를 해주면 일단 걸러진다..
			------------------------------------------------------------------------------------------*/
			// 확실한 Seperating Axe 찾았으면 충돌 검사할 필요가 없이 충돌하지 않은 상태이다.
			if (FindSeperatingAxe_BetweenTriangleSphere(triangle.vertices, target, radius))
			{
				return CollisionInfo();
			}

			//역방향 충돌 모두 제거 - 벽 같은 경우는 플레이어가 벽을 뚫고 지나가려고 하면 역방향 충돌이 발생해서 벽 건너편으로 순간이동 현상 발생
			// History - Terrain이 엄청 촘촘하고 울퉁불퉁한 경우에는 플레이어가 방향키 이동을 했을 때 Normal의 반대방향으로 가서 땅이 뚫고 내려갈 수 있어서 주석 처리.
			...
			{
				// Sphere가 라인에 걸쳐있는 경우에는 sphere.radius - line 까지의 거리가 penetraion depth가 되어야 한다.
				{
		...
		{
			// 물체가 지형과 충돌해서 발생한 Normal 중 ↖ 와 ↗ 는 경사로에서 발생하는 충돌이니 미끄럼 방지를 위해 따로 별도 처리 해준다
			// ↙ 하고 ↘ 방향 충돌은 점프를 해서 어딘가 부딪혔을 때이기 때문에 y > 0.0f 로 기준을 잡았다
			...
			if (IsCollidedFromSlope(collisionInfo))
			{
				...
				// new Vector3(0f, 1f, 0f)로 해도 되고 new Vector3(0f, temp.y, 0f) 로 해도 같은 값이 나와야한다
				...
				// 삼각비의 법칙 => a / sin(반대편 각) = b / sin(반대편 각) = c / sin (반대편 각)
				// (빗변 c 의 길이 / Sin(90)) == (penetrationDepth / sin(반대편 각))
				// Sin(90)은 항상 1이니 최종 공식은 => 빗변 c 의 길이 == penetrationDepth / sin(반대편 각)  
				// float 빗변의 길이 = (collisionInfo.penetrationDepth / sin(anotherAngleInDegree)); 
				// sin() 함수는 degree가 아닌 radian을 인자로 받으므로 Mathf.Deg2Rad 를 곱해준다. 
				...
				// 어떻게 빗변의 크기보다 클 수 있지...?
				// newDepth 는 빗변의 길이이므로 삼각형의 그 어떠한 line의 길이도 빗변보다 클 수가 없다.
		...
		// Vector3 A와 B 를 projectTo(normal) 기준으로 overlap 계산을 하는데 정방향 overlap인지 체크
		// normal -> A B가 정방향(forward)
		// normal -> B A는 역방향(backward)
		static float CalculateOverlap_BetweenPointSphere(Vector3 projectTo, Vector3 targetA, Vector3 targetB, float radius)
		{
			...
			// crossProduct와 vertex 사이의 거리를 구한다(cross product 과 vertex[0, 1, 2] 사이의 Dot product 는 모두 동일)
	...
	private:
		...
		static float IsSphereHangingOnLineOfTriangle(Triangle t, Vector3 target, float radius)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				// Normal 을 구할 때 - vertices[i] 해줬으니 Target 도 - vertices[i] 해줘야하고
				// Perpendicular 구한 뒤 + vertices[i] 해줘야한다.
				{
					...
					// History - B_Dot = 0.98468, pernDot = 0.98455 일 때 에디터 상 충돌을 한 것 처럼 보이는데
					// B_Dot < pernDot 연산에 의해서 return 0f가 되고 있어서 float decial의 3번째 자리까지 같은 경우는 같은 값으로 간주
		...
		{
			// 방향 백터 [A-B, B-C, C-A]와 방향 백터[A-P, B-P, C-P] 가 같은 방향을 가르키는지 체크
			// 모든 방향 백터가 같은 방향을 가르킨다면 점 P는 Triangle 안에 있다
	...
	public:
		...
		// Version 2 - 위치를 기반으로 GridIndex를 알아내고 해당 Grid에 인접한 다른 Grid 들에 속하는 Mesh 중 인접한 Triangle 들만 체크한다.
		// Dependency 1 - GetBestCollisionInfo_From_Terrain : 충돌한 정보 중 가장 정확한 정보만 처리
		// Dependency 2 - GetBestCollisionInfo_From_Object : 충돌한 정보 중 DotProductType 별로 가장 정확한 정보들만 처리
	...
	private:
		CollisionInfo GetBestCollisionInfo_From_Terrain(Mesh mesh, vector<int> triangleIndicesToCheck, Vector3 position, float radius)
		{
			...
			// 경사의 미끄러짐을 방지한다면 충돌이 경사로에서 발생했는지 체크 후 resolve 해준다
		...
		// Object 와의 충돌한 정보 중 DotProductType 별로 가장 정확한 충돌 정보를 리턴한다.
		...
		{
			// Ver1 - Vector3(1,0,0) 와 DotProduct를 계산해서 음수, 0, 양수에 해당하는 normal들의 충돌 정보를 List 에 모은다.(비슷한 방향에서 발생한 충돌 정보 중 가장 정확한 것만 적용하기 위함
			...
			for (int triangleIndex : triangleIndicesToCheck)
			{
				...
				if (collisionInfo.IsCollided())
				{
					...
					// 여기서 2가지 방법의 장단점이 있다.
					// 방법 1 : 충돌한 Normal 방향들 중 비슷한 방향들을 그룹으로 묶은 뒤 Normal 별로 가장 정확한 Collision Info를 찾기 위함
					// 장점 : 건물 내부의 벽에서 밖으로 나가려고 할 때 jittering이 발생하지 않는다 왼벽 앞벽의 충돌을 모두 처리해주기 때문, 방법 2로 하면 jiterring 발생
					//
					// 방법 2 : Normal 방향 상관없이 Mesh에서 가장 정확한 Collision Info 단 하나만 찾기
					// 방법 2 단점 : 여러 물체와 충돌해서 꼈을 때 jittering 발생, 모서리에 끼었다가 빠져나갈 때 가속도가 붙는 느낌이 있다

					// HISTORY - 모두 DotProductType::NONE 를 key로 넣어준 이유는 하나의 Mesh에서 발생한 단 하나의 Collision Info만 체크해줘보자
			...
			// 같은 방향(normal)으로 충돌한 정보 중 가장 정확한 정보만 찾는다.(같은 방향(normal)으로 충돌이 발생 했을 경우 가장 정확한 충돌 방향은 penetrationDepth가 가장 큰 값일 때 이다)
